...
def handler(event, context):
    ...
    response = requests.get('https://threatview.io/Downloads/DOMAIN-High-Confidence-Feed.txt', headers=headers, timeout=60)
    print(f'HTTP Status Code: {response.status_code}')
    data = response.text

    fname = f'{year}-{month}-{day}-{hour}-{minute}-threatview.csv'
