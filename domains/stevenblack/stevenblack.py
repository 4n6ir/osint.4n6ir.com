...
def handler(event, context):
    ...
    response = requests.get('https://raw.githubusercontent.com/StevenBlack/hosts/master/hosts', headers=headers, timeout=60)
    print(f'HTTP Status Code: {response.status_code}')
    data = response.text

    fname = f'{year}-{month}-{day}-{hour}-{minute}-stevenblack.csv'
